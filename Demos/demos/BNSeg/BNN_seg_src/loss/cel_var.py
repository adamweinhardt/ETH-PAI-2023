...
class CEL_VAR(nn.Module):
    ...
    def gaussian_categorical_cross_entropy(self,mean,std,label,num_mc_sample=5):
        """
        batch x classes x height x width
        each pixel have one mean, std, sample num_mc_sample times
        """
        sample_size = std.size()
        ...
        for sample_idx in range(num_mc_sample):
            ...
            distorted_loss_list[sample_idx]  = F.cross_entropy(logits_sample, label)
