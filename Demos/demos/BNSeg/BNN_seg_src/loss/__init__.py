...
class Loss(nn.Module):
    def __init__(self, config):
        ...
        if config.uncertainty == 'epistemic' or config.uncertainty == 'normal':
            module = import_module('loss.cel') 
            loss_function = getattr(module, 'CEL')()
        elif config.uncertainty == 'aleatoric' or config.uncertainty == 'combined':
            module = import_module('loss.cel_var')
            loss_function = getattr(module, 'CEL_VAR')(
